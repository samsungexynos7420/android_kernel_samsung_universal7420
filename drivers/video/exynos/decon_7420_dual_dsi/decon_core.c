...
int decon_enable_lpd(struct decon_device *decon)
{
...
#endif
	if (decon->id == 0 && !decon->eint_status) {
		if (decon->eint_pend) {
	...
	decon_reg_set_int(decon->id, &psr, DSI_MODE_DUAL, 1);
...
int decon_enable(struct decon_device *decon)
{
	...
	if (decon->pdata->dsi_mode == DSI_MODE_DUAL) {
		decon_info("enable 2nd DSIM for dual DSI mode\n");
...
int __decon_exit_lpd(struct decon_device *decon)
{
	...
	dsim[0] = container_of(decon->output_sd, struct dsim_device, sd);
	dsim[1] = container_of(decon->output_sd1, struct dsim_device, sd);
...
#if defined(CONFIG_DECON_DEVFREQ)
	exynos7_update_media_scenario(TYPE_DECON_INT, decon->default_bw, 0);
	pm_qos_update_request(&decon->disp_qos, 167000);
	pm_qos_update_request(&decon->int_qos, 167000);
	decon->prev_bw = decon->default_bw;
	decon->prev_int_bw = 167000;
	...
	if (dsim[0]->state == DSIM_STATE_ULPS) {
		ret = v4l2_subdev_call(decon->output_sd, core, ioctl,
				DSIM_IOC_ENTER_ULPS, (unsigned long *)0);
		if (ret) {
			decon_err("%s: failed to exit ULPS state for %s\n",
	...
	if (dsim[1]->state == DSIM_STATE_ULPS) {
	...
	if (!decon->eint_status) {
		if (dsim[0]->state == DSIM_STATE_HSCLKEN) {
			if (decon->eint_pend) {
				te_pending = readl(decon->eint_pend);
				writel(te_pending | decon->eint_pend_mask, decon->eint_pend);
			}
			...
			if (dsim[1]->state == DSIM_STATE_HSCLKEN)
				enable_irq(decon->irq_te_sub);
		}
		DISP_SS_EVENT_LOG(DISP_EVT_GIC_TE_ENABLE, &decon->sd, ktime_set(0, 0));
		decon->eint_status = 1;
	}
...
int __decon_enter_lpd(struct decon_device *decon)
{
	...
	if (decon->state == DECON_STATE_LPD) {
		decon_info("decon is already LPD state.\n");
		goto err;
	}
	...
	decon_reg_set_int(decon->id, &psr, DSI_MODE_DUAL, 0);

	if ((decon->vsync_info.irq_refcount <= 0) && decon->eint_status) {
		if (dsim[0]->state == DSIM_STATE_HSCLKEN) {
			disable_irq(decon->irq);
		} else {
			if (dsim[1]->state == DSIM_STATE_HSCLKEN)
				disable_irq(decon->irq_te_sub);
		}
...
int decon_disable_lpd(struct decon_device *decon)
{
	...
	if ((decon->state == DECON_STATE_LPD) || (decon->state == DECON_STATE_OFF)) {
		decon_info("decon%d already lpd\n", decon->id);
		goto err;
	}
...
int decon_disable(struct decon_device *decon)
{
	...
	if (decon->pdata->dsi_mode == DSI_MODE_DUAL) {
		decon_info("disable 2nd DSIM for dual DSI\n");
	...
	decon->need_disp_on = DECON_DISPLAY_ON_REQ;
...
static int decon_blank(int blank_mode, struct fb_info *info)
{
	...
	decon_info("decon-%s %s mode: %dtype (0: DSIM0, 1: HDMI, 2:WB, 3:DSIM1)\n",
			decon->id ? "ext" : "int",
			blank_mode == FB_BLANK_UNBLANK ? "UNBLANK" : "POWERDOWN",
			decon->out_type);
	decon_info("%d dsi mode (0: SINGLE DSI, 1: DUAL DSI, 2: DUAL DISPLAY\n",
	...
	case FB_BLANK_UNBLANK:
		...
		if ((decon->id == 1) && (decon->state == DECON_STATE_OFF)) {
			ret = decon_enable(decon_int_drvdata);
...
//                        1080 ~        -> update for dsi1 (sub)
	if (win_config->dst.x < decon->lcd_info->xres)
		regs->which_dsi |= WIN_UPDATE_DSI0;
	else
		regs->which_dsi |= WIN_UPDATE_DSI1;
#endif
	if (!is_vpp_type(win_config->idma_type)) {
	...
	decon_dbg("win[%d] VPP:%d SRC:(%d,%d) %dx%d  DST:(%d,%d) %dx%d\n", win_no,
			is_vpp_type(win_config->idma_type),
...
static void decon_displayon_req(struct decon_device *decon, struct decon_reg_data *regs)
{
	int ret = 0;
	unsigned int flag;
	struct dsim_device *dsim = NULL;
	...
	if (decon->out_type != DECON_OUT_DSI)
		goto exit_displayon_req;

	if ((regs->which_dsi & WIN_UPDATE_DSI0) && (decon->output_sd != NULL)) {
		dsim = container_of(decon->output_sd, struct dsim_device, sd);
		panel = &dsim->priv;
		if (CHECK_NEED_DISPLAY_ON(panel->op_state)) {
			decon_info("DECON:%s:display on req to dsim:0\n", __func__);
			ret = v4l2_subdev_call(decon->output_sd, core, ioctl,
				DSIM_IOC_DISPLAY_ON, (unsigned long *)0);
			if (ret) {
				decon_err("ERR:DECON:%s:failed to display-on dsim : 0\n", __func__);
				goto exit_displayon_req;
			}
		}
	}

	if ((regs->which_dsi & WIN_UPDATE_DSI1) && (decon->output_sd1 != NULL)) {
		dsim = container_of(decon->output_sd1, struct dsim_device, sd);
		panel = &dsim->priv;
		if (CHECK_NEED_DISPLAY_ON(panel->op_state)) {
			decon_info("DECON:%s:display on req to dsim:1\n", __func__);
			ret = v4l2_subdev_call(decon->output_sd1, core, ioctl,
				DSIM_IOC_DISPLAY_ON, (unsigned long *)0);
			if (ret) {
				decon_err("ERR:DECON:%s:failed to display-on dsim : 0\n", __func__);
	...
	/* for dual dsi */
	if ((decon->output_sd != NULL) && (decon->output_sd1 != NULL)) {
		dsim = container_of(decon->output_sd, struct dsim_device, sd);
		panel = &dsim->priv;
		flag = panel->op_state;
		...
		if (CHECK_ALREADY_DISPLAY_ON(flag) && CHECK_ALREADY_DISPLAY_ON(panel->op_state))
			decon->need_disp_on = DECON_DISPLAY_ON_DONE;

	/* for single dsi */
	} else if (decon->output_sd != NULL) {
		dsim = container_of(decon->output_sd, struct dsim_device, sd);
		panel = &dsim->priv;

		if (CHECK_ALREADY_DISPLAY_ON(panel->op_state)) {
			decon->need_disp_on = DECON_DISPLAY_ON_DONE;
		}
	}
exit_displayon_req:
...
static void decon_update_regs(struct decon_device *decon, struct decon_reg_data *regs)
{
	...
	if ((decon->need_disp_on == DECON_DISPLAY_ON_REQ) 
			&& (decon->out_type == DECON_OUT_DSI)) {
		decon_displayon_req(decon, regs);
	}
...
/* ---------- SYSTEM REGISTER CONTROL ----------- */
static void decon_swap_te_input(struct decon_device *decon, unsigned int enable)
{
	u32 val = readl(decon->sys_regs + DISP_CFG);

	if (enable)
		val |= DISP_CFG_DECON_INT_TE_SWAP;
	else 
		val &= ~(DISP_CFG_DECON_INT_TE_SWAP);
...
unsigned int dual_fsm[REQ_MAX][CUR_MAX] = {
	{
		DECON_ON | TE_REF_MAIN | DSIM0_MASTER | DSIM1_SLAVE,
		DECON_ON | TE_REF_MAIN | DSIM0_STANDALONE | DSIM0_OFF,
		DECON_ON | TE_NEED_UPDATE | TE_REF_MAIN | DSIM0_NEED_UPDATE | DSIM0_MASTER | DSIM1_NEED_UPDATE | DSIM1_SLAVE,
		DECON_NEED_UPDATE | DECON_ON | TE_NEED_UPDATE | TE_REF_MAIN | DSIM0_NEED_UPDATE | DSIM0_STANDALONE | DSIM1_OFF,
	},
	{
		DECON_ON | TE_REF_MAIN | DSIM0_MASTER | DSIM1_SLAVE,
		DECON_ON | TE_NEED_UPDATE | TE_REF_MAIN | DSIM0_NEED_UPDATE | DSIM0_MASTER | DSIM1_NEED_UPDATE | DSIM1_SLAVE,
		DECON_ON | TE_REF_SUB | DSIM0_OFF | DSIM1_STANDALONE,
		DECON_NEED_UPDATE | DECON_ON | TE_NEED_UPDATE | TE_REF_SUB | DSIM0_OFF | DSIM1_NEED_UPDATE | DSIM1_STANDALONE,
	},
	{
		DECON_ON | TE_NEED_UPDATE | TE_REF_SUB | DSIM0_NEED_UPDATE | DSIM0_OFF | DSIM1_NEED_UPDATE | DSIM1_STANDALONE, 
		DECON_NEED_UPDATE | DECON_OFF | TE_OFF | DSIM0_NEED_UPDATE | DSIM0_OFF | DSIM1_OFF,
		DECON_ON | TE_REF_SUB | DSIM0_OFF | DSIM1_STANDALONE,
		DECON_OFF | TE_OFF | DSIM0_OFF | DSIM1_OFF,
	},
	{
		DECON_ON | TE_REF_MAIN | DSIM0_NEED_UPDATE | DSIM0_STANDALONE | DSIM1_NEED_UPDATE | DSIM1_OFF,
		DECON_ON | TE_REF_MAIN | DSIM0_STANDALONE | DSIM1_OFF,
		DECON_NEED_UPDATE | DECON_OFF | TE_OFF | DSIM0_OFF | DSIM1_NEED_UPDATE | DSIM0_OFF,
		DECON_OFF | TE_OFF | DSIM0_OFF | DSIM1_OFF,
	},
...
static int decon_dual_dsi_enable(struct decon_device *decon, struct dual_blank_data *data)
{
	int ret = 0;
	int alpm = 0;
	struct decon_psr_info psr;
	...
	if (data->type == DECON_PRIMARY_DISPLAY) {
		ret = v4l2_subdev_call(decon->output_sd, video, s_stream, 1);
		if (ret) {
			decon_err("starting stream failed for %s\n", decon->output_sd->name);
			goto err;
	...
	if (!decon->eint_status) {
		if (data->type == DECON_PRIMARY_DISPLAY) {
			...
			enable_irq(decon->irq);
			decon_swap_te_input(decon, DECON_TE_SWAP_DISABLE);
		} else {
			enable_irq(decon->irq_te_sub);
			decon_swap_te_input(decon, DECON_TE_SWAP_ENABLE);
		}
		DISP_SS_EVENT_LOG(DISP_EVT_GIC_TE_ENABLE, &decon->sd, ktime_set(0, 0));
		decon->eint_status = 1;
	}
...
static int decon_dual_dsi_disable(struct decon_device *decon, struct dual_blank_data *data)
{
	int ret = 0;
	struct decon_psr_info psr;
	//int i, j;
...
#define DECON_ENABLE_REQ	0x00
#define DECON_DISABLE_REQ	0x01


unsigned int decon_dual_dsi_get_next_stat(struct decon_device *decon, struct dual_blank_data *data)
{
	unsigned int cur, req;
	unsigned int req_idx, cur_idx;	

	cur = decon->dual_dsi_stat;

	switch (data->blank) {
		case FB_BLANK_POWERDOWN:
		case FB_BLANK_NORMAL:
			DISP_SS_EVENT_LOG(DISP_EVT_BLANK, &decon->sd, ktime_set(0, 0));
			req = DECON_DISABLE_REQ;
			break;
		case FB_BLANK_UNBLANK:
			DISP_SS_EVENT_LOG(DISP_EVT_UNBLANK, &decon->sd, ktime_set(0, 0));
			req = DECON_ENABLE_REQ;
			break;
		case FB_BLANK_VSYNC_SUSPEND:
		case FB_BLANK_HSYNC_SUSPEND:
	...
	req_idx = data->type | req << 1;
	cur_idx = ((GET_DSIM0_STAT(cur) == DSIM_OFF) ? (0x1 << 1) : 0) |
				((GET_DSIM1_STAT(cur)  == DSIM_OFF) ? 1 : 0);

	if ((req_idx >= REQ_MAX) || (cur_idx >= CUR_MAX)) {
		decon_err("ERR:DECON:%s:wrong idx (req : %d, cur :%d)\n", __func__, req_idx, cur_idx);
		goto err;
	}
	return dual_fsm[req_idx][cur_idx];
...
static void print_dsi_stat(unsigned int stat, char *string)
{
	char *dsim_status[4] = {
		"OFF",
		"MASTER",
	...
	decon_info("=========== %s : DSI STAT ===========\n", string);
	if (stat & DECON_NEED_UPDATE) {
		if (stat & DECON_ON)
			decon_info("decon need to update : decon on\n");
		else
			decon_info("decon need to update : decon off\n"); 
	} else {
		if (stat & DECON_ON)
			decon_info("decon on\n");
		else
			decon_info("decon off\n"); 
	}
	
	if (stat & TE_NEED_UPDATE) {
		if (stat & TE_REF_MAIN)
			decon_info("te need to update : te ref main\n");
		else if (stat & TE_REF_SUB)
			decon_info("te need to update : te ref sub\n");
		else 
			decon_info("te need to update : te off\n");
	} else {
		if (stat & TE_REF_MAIN)
			decon_info("te ref main\n");
		else if (stat & TE_REF_SUB)
			decon_info("te ref sub\n");
		else 
			decon_info("te off\n");
	}

	if (stat & DSIM0_NEED_UPDATE) {
		decon_info("dsim0 need to update : %s\n", dsim_status[GET_DSIM0_STAT(stat)]);
	} else {
		decon_info("dsim0 %s\n", dsim_status[GET_DSIM0_STAT(stat)]);
	}

	if (stat & DSIM1_NEED_UPDATE) {
		decon_info("dsim1 need to update : %s\n", dsim_status[GET_DSIM1_STAT(stat)]);
	} else {
		decon_info("dsim1 %s\n", dsim_status[GET_DSIM1_STAT(stat)]);
	}
}
...
int decon_dual_dsi_blank(struct decon_device *decon, struct dual_blank_data *data)
{
	int ret = 0;
	int next_stat;
	int flag = 0;
	...
	decon_info("%s id - %d ++ blank_mode : %d \n", __func__, data->type ,data->blank);
	...
	next_stat = decon_dual_dsi_get_next_stat(decon, data);
	print_dsi_stat(decon->dual_dsi_stat, "current");
	print_dsi_stat(next_stat, "next");

	if (next_stat == decon->dual_dsi_stat) {
		decon_info("DECON:%s:already set to %x\n", __func__, decon->dual_dsi_stat);
		goto blank_exit;
	}

	if (decon->ignore_vsync == 0) {
		flag = 1;
	...
	if (decon->state == DECON_STATE_ON) {
		...
		vstatus = decon_reg_get_vstatus(decon->id, 0);
		ret = decon_reg_wait_linecnt_is_zero_timeout(decon->id, 0, 50 * 1000);
		if (ret) {
			decon_err("ERR:DECON:%s:timeout wait line count\n", __func__);
		}
		decon_vpp_stop(decon, true);
	}
#endif
	if (next_stat & DECON_NEED_UPDATE) {
		if (CHECK_DECON_ON(next_stat)) {
			ret = v4l2_subdev_call(decon->output_sd, video, s_stream, 1);
			if (ret) {
				decon_err("ERR:DECON:%s:failed to dsim0 enable\n", __func__);
				goto blank_exit;
			}
			...
			ret = decon_dual_dsi_enable(decon, data);
			if (ret) {
				decon_err("ERR:DECON:%s:failed to decon enable\n", __func__);
				goto blank_exit;

			}
		} else {
			ret = decon_dual_dsi_disable(decon, data);
			if (ret) {
				decon_err("ERR:DECON:%s:failed to decon disable\n", __func__);
				goto blank_exit;
			}
			ret = v4l2_subdev_call(decon->output_sd1, video, s_stream, 0);
			if (ret) {
		...
		decon->dual_dsi_stat = next_stat;
		goto blank_exit;
	}

#if 0 
	if (next_stat & TE_NEED_UPDATE) {
		if (decon->dual_dsi_stat & TE_REF_MAIN)
			disable_irq(decon->irq);
		else if (decon->dual_dsi_stat & TE_REF_SUB)
			disable_irq(decon->irq_te_sub);

		if (next_stat & TE_REF_MAIN) {
			decon_swap_te_input(decon, DECON_TE_SWAP_DISABLE);
		} else if (next_stat & TE_REF_SUB) {
			decon_swap_te_input(decon, DECON_TE_SWAP_ENABLE);
		}
	}
	...
	if (next_stat & DECON_NEED_UPDATE) {
		...
		if (next_stat & TE_NEED_UPDATE) {
			decon_info("reset decon\n");
			decon_info("Curr : TE %x\n", decon->dual_dsi_stat & (TE_REF_MAIN|TE_REF_SUB));
			decon_info("Next : TE %x\n", next_stat & (TE_REF_MAIN|TE_REF_SUB));
	...
	if (next_stat & DSIM0_NEED_UPDATE) {
		if (GET_DSIM0_STAT(next_stat) == DSIM_OFF) {
			ret = v4l2_subdev_call(decon->output_sd, video, s_stream, 0);
			if (ret) {
				decon_err("ERR:DECON:%s:failed to dsim0 disable\n", __func__);
				goto blank_exit;
			}
		} else {
			ret = v4l2_subdev_call(decon->output_sd, video, s_stream, 1);
	...
	if (next_stat & DSIM1_NEED_UPDATE) {
		if (GET_DSIM1_STAT(next_stat) == DSIM_OFF) {
			ret = v4l2_subdev_call(decon->output_sd1, video, s_stream, 0);
			if (ret) {
				decon_err("ERR:DECON:%s:failed to dsim1 disable\n", __func__);
				goto blank_exit;
			}
		} else {
			ret = v4l2_subdev_call(decon->output_sd1, video, s_stream, 1);
			if (ret) {
				decon_err("ERR:DECON:%s:failed to dsim1 enable\n", __func__);
				goto blank_exit;
			}
		}
	}
			...
			unsigned long arg)
{
	...
	case S3CFB_DUAL_DISPLAY_BLANK:
		...
		ret = decon_dual_dsi_blank(decon, &blank_data);
		break;
#endif
	default:
		ret = -ENOTTY;
...
static int decon_register_entity(struct decon_device *decon)
{
	...
	if (!decon->id) {
		...
		if (decon->pdata->out_type[0] == DECON_OUT_DSI1)
			ret = find_subdev_mipi(decon);
		else if (decon->pdata->out_type[0] == DECON_OUT_WB)
			ret = find_subdev_wb(decon);
#ifdef CONFIG_VIDEO_EXYNOS_HDMI
		else
...
static void decon_parse_pdata(struct decon_device *decon, struct device *dev)
{
	...
	if (dev->of_node) {
		...
		/* output device type */
		of_property_read_u32_index(dev->of_node, "out_type", 0,
				&decon->pdata->out_type[0]);
		decon_info("1st output device type(%d). 0: DSIM0 1: HDMI 2: WB 3: TUI 4: DSIM1\n",
				decon->pdata->out_type[0]);

		decon->out_type = decon->pdata->out_type[0];

		if (!decon->id && (decon->pdata->dsi_mode == DSI_MODE_DUAL)) {
			of_property_read_u32_index(dev->of_node, "out_type", 1,
					&decon->pdata->out_type[1]);
			decon_info("2nd output device type(%d). 0: DSIM0 1: HDMI 2: WB 3: TUI 4: DSIM1\n",
					decon->pdata->out_type[1]);
		}
...
decon_rest_init:
	...
	decon->force_fullupdate = 0;
	decon->need_disp_on = DECON_DISPLAY_ON_REQ;
	decon->dual_dsi_stat = DECON_ON | TE_REF_MAIN | DSIM0_MASTER | DSIM1_SLAVE;
