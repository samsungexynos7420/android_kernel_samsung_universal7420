...
void decon_int_set_clocks(struct decon_device *decon)
{
	...
	case (1920 * 1080):
		...
		decon_clk_set_parent(dev, "mout_sclk_decon_int_eclk", "mout_bus0_pll_top0");
		if (decon->pdata->dsi_mode == DSI_MODE_DUAL)
			decon_clk_set_rate(dev, "dout_sclk_decon_int_eclk", 200 * MHZ);
		else
		...
		if (decon->pdata->dsi_mode == DSI_MODE_DUAL)
			decon_clk_set_rate(dev, "d_decon0_eclk", 200 * MHZ);
		else
			decon_clk_set_rate(dev, "d_decon0_eclk", 100 * MHZ);

		/* VCLK(142Mhz) : using DISP PLL */
		decon_clk_set_rate(dev, "disp_pll", 143 * MHZ);
		decon_clk_set_parent(dev, "m_decon0_vclk", "disp_pll");
	...
	decon_info("%s:disp_pll %ld d_decon0_eclk %ld d_decon0_vclk %ld dout_aclk_disp_400 %ld d_pclk_disp %ld\n",
...
int find_subdev_mipi(struct decon_device *decon)
{
	...
	if (decon->pdata->dsi_mode == DSI_MODE_DUAL) {
		decon->output_sd1 = md->dsim_sd[1];

		if (IS_ERR_OR_NULL(decon->output_sd1))
			decon_warn("couldn't find dsim1 subdev\n");
	}
...
int decon_pan_display(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	if ((decon->need_disp_on == DECON_DISPLAY_ON_REQ) && (decon->output_sd != NULL)) {
...
int decon_fb_config_eint_for_te(struct platform_device *pdev, struct decon_device *decon)
{
	...
	gpio = gpio_to_irq(gpio);
	decon->irq_te_sub = gpio;
	ret = devm_request_irq(dev, gpio, decon_fb_isr_for_eint,
			  IRQF_TRIGGER_RISING, "TE SUB", decon);
...
#if 0 
static ssize_t sub_lcd_power_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
...
extern int decon_dual_dsi_blank(struct decon_device *decon, struct dual_blank_data *data);

static ssize_t sub_lcd_power_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	switch (value) {
		case 0 : 
			/* sub lcd off */
			data.type = DECON_SECONDARY_DISPLAY;
			data.blank = FB_BLANK_POWERDOWN;
			ret = decon_dual_dsi_blank(decon, &data);
			break;
		case 1 : 
			/* sub lcd on */
			data.type = DECON_SECONDARY_DISPLAY;
			data.blank = FB_BLANK_UNBLANK;
			ret = decon_dual_dsi_blank(decon, &data);
			break;
		case 2 :
			data.type = DECON_PRIMARY_DISPLAY;
			data.blank = FB_BLANK_POWERDOWN;
			ret = decon_dual_dsi_blank(decon, &data);
			break;
		case 3 : 
...
static DEVICE_ATTR(sub_lcd, 0664, sub_lcd_power_show, sub_lcd_power_store);
#endif
...
#if 0 
	ret = device_create_file(decon->dev, &dev_attr_sub_lcd);
	if (ret) {
		decon_err("failed to create sub lcd file\n");
		return ret;
	}
#endif
	return ret;
