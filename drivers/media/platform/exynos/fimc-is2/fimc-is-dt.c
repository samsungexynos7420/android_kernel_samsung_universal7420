...
static int parse_dvfs_data(struct exynos_platform_fimc_is *pdata, struct device_node *np, int index)
{
	...
	if (index >= FIMC_IS_DVFS_TABLE_IDX_MAX) {
		probe_err("dvfs table index is invalid(%d)", index);
		return -EINVAL;
	}
	...
	DT_READ_U32(np, "rear_video_whd_int", pdata->dvfs_data[index][FIMC_IS_SN_REAR_CAMCORDING_WHD][FIMC_IS_DVFS_INT]);
	DT_READ_U32(np, "rear_video_whd_cam", pdata->dvfs_data[index][FIMC_IS_SN_REAR_CAMCORDING_WHD][FIMC_IS_DVFS_CAM]);
	DT_READ_U32(np, "rear_video_whd_mif", pdata->dvfs_data[index][FIMC_IS_SN_REAR_CAMCORDING_WHD][FIMC_IS_DVFS_MIF]);
	DT_READ_U32(np, "rear_video_whd_i2c", pdata->dvfs_data[index][FIMC_IS_SN_REAR_CAMCORDING_WHD][FIMC_IS_DVFS_I2C]);
	...
	DT_READ_U32(np, "rear_video_whd_capture_int", pdata->dvfs_data[index][FIMC_IS_SN_REAR_CAMCORDING_WHD_CAPTURE][FIMC_IS_DVFS_INT]);
	DT_READ_U32(np, "rear_video_whd_capture_cam", pdata->dvfs_data[index][FIMC_IS_SN_REAR_CAMCORDING_WHD_CAPTURE][FIMC_IS_DVFS_CAM]);
	DT_READ_U32(np, "rear_video_whd_capture_mif", pdata->dvfs_data[index][FIMC_IS_SN_REAR_CAMCORDING_WHD_CAPTURE][FIMC_IS_DVFS_MIF]);
	DT_READ_U32(np, "rear_video_whd_capture_i2c", pdata->dvfs_data[index][FIMC_IS_SN_REAR_CAMCORDING_WHD_CAPTURE][FIMC_IS_DVFS_I2C]);
	...
	DT_READ_U32(np, "preview_high_speed_fps_int", pdata->dvfs_data[index][FIMC_IS_SN_PREVIEW_HIGH_SPEED_FPS][FIMC_IS_DVFS_INT]);
	DT_READ_U32(np, "preview_high_speed_fps_cam", pdata->dvfs_data[index][FIMC_IS_SN_PREVIEW_HIGH_SPEED_FPS][FIMC_IS_DVFS_CAM]);
	DT_READ_U32(np, "preview_high_speed_fps_mif", pdata->dvfs_data[index][FIMC_IS_SN_PREVIEW_HIGH_SPEED_FPS][FIMC_IS_DVFS_MIF]);
	DT_READ_U32(np, "preview_high_speed_fps_i2c", pdata->dvfs_data[index][FIMC_IS_SN_PREVIEW_HIGH_SPEED_FPS][FIMC_IS_DVFS_I2C]);
...
int fimc_is_parse_dt(struct platform_device *pdev)
{
	...
	while ((dvfs_table_np = of_get_next_child(dvfs_np, dvfs_table_np))) {
		...
		probe_info("dvfs table[%d] %s", table_index, dvfs_table_desc);
		dvfs_table_desc = NULL;
		ret = parse_dvfs_data(pdata, dvfs_table_np, table_index);
		if (ret) {
			err("parse_dvfs_data is fail(%d, idx:%d)", ret, table_index);
		}
