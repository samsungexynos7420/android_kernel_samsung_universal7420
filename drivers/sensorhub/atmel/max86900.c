...
static int max86900_regulator_onoff(struct max86900_device_data *data, int onoff)
{
	int err;
	if (data->vldo39 != NULL)
		data->vdd_1p8 = regulator_get(NULL, data->vldo39);
	else
	...
	if (data->vldo40 != NULL)
		data->vdd_3p3 = regulator_get(NULL, data->vldo40);
	else
		data->vdd_3p3 = regulator_get(NULL, "V_IRLED_3.3V");
	...
	struct device *dev)
{
	...
	if (of_property_read_string(dNode, "max86900,hrm_vdd", &data->vldo39) < 0)
		pr_err("%s - get vldo39 error\n", __func__);

	if (of_property_read_string(dNode, "max86900,hrm_vled", &data->vldo40) < 0)
		pr_err("%s - get vldo40 error\n", __func__);

	pr_info("%s - %s, %s\n", __func__, data->vldo39, data->vldo40);
	return 0;
}
