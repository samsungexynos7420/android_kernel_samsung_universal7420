...
TRACE_EVENT(exynos_hpgov_governor_update,
	    TP_PROTO(int hp_event, int req_cpu_max, int req_cpu_min),
	    TP_ARGS(hp_event, req_cpu_max, req_cpu_min),
	    TP_STRUCT__entry(
		    __field(int, hp_event)
		    __field(int, req_cpu_max)
		    __field(int, req_cpu_min)
	    ),
	    TP_fast_assign(
		    __entry->hp_event = hp_event;
		    __entry->req_cpu_max = req_cpu_max;
		    __entry->req_cpu_min = req_cpu_min;
	    ),
	    TP_printk("hp_event=%d req_cpu_max=%d req_cpu_min=%d",
		    __entry->hp_event, __entry->req_cpu_max, __entry->req_cpu_min)
);
